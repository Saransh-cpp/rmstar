...
def test_get_names_from_dir(tmpdir, relative):
    ...
    try:
        ...
        assert (
            get_names_from_dir(".submod_recursive.submod2", directory)
        ...
        assert (
            get_names_from_dir("module.submod_recursive.submod2", directory)
            == submod_recursive_submod2_names
        )
        ...
        assert (
            get_names_from_dir("..submod_recursive.submod2", submod)
            == submod_recursive_submod2_names
        )
        ...
        assert (
            get_names_from_dir("module.submod_recursive.submod2", submod)
            == submod_recursive_submod2_names
        )
        ...
        assert (
            get_names_from_dir("module.submod_recursive.submod1", submod_recursive)
            == submod_recursive_submod1_names
        )
        assert (
...
def test_get_names_dynamically(tmpdir):
    ...
    try:
        ...
        assert (
            get_names_dynamically("module.submod_recursive.submod2")
            == submod_recursive_submod2_dynamic_names
        )
...
def test_fix_code(tmpdir, capsys):
    ...
    assert (
        fix_code(
            code_mod_commented_unused_star,
            file=directory / "mod_commented_unused_star.py",
        )
        == code_mod_commented_unused_star_fixed
    )
    out, err = capsys.readouterr()
    ...
    assert "Warning" in err
    assert (
        "The removed star import statement for '.mod1' had an inline "
        "comment which may not make sense without the import"
    ) in err

    assert (
        fix_code(code_mod_commented_star, file=directory / "mod_commented_star.py")
        == code_mod_commented_star_fixed
    )
    out, err = capsys.readouterr()
    assert not out
    ...
    assert (
        fix_code(code_submod_recursive_submod1, file=submod_recursive / "submod1.py")
        == code_submod_recursive_submod1
    )
    out, err = capsys.readouterr()
    assert not out
    ...
    assert (
        fix_code(code_submod_recursive_submod2, file=submod_recursive / "submod2.py")
        == code_submod_recursive_submod2_fixed
    )
    out, err = capsys.readouterr()
    assert not out
...
def test_replace_imports():
    ...
    assert (
        replace_imports(
            code_mod5,
            repls={"module.mod1": ["a"], "module.mod2": ["b", "c"]},
            verbose=False,
            quiet=True,
        )
        == code_mod5_fixed
    ...
    assert (
        replace_imports(
            code_mod6, repls={"os.path": ["isfile", "join"]}, verbose=False, quiet=False
        )
        == code_mod6_fixed
    )
    assert (
        replace_imports(code_mod7, repls={".mod6": []}, verbose=False, quiet=False)
        == code_mod7_fixed
    )
    assert (
        replace_imports(
    ...
    assert (
        replace_imports(
            code_submod1,
            repls={"..mod1": ["a"], "..mod2": ["b", "c"], ".submod3": ["e"]},
            verbose=False,
            quiet=True,
        )
        == code_submod1_fixed
    ...
    assert (
        replace_imports(code_submod4, repls={".": ["func"]}, verbose=False, quiet=True)
        == code_submod4_fixed
    )

    assert (
        replace_imports(code_submod_recursive_submod2, repls={".": ["a"]})
        == code_submod_recursive_submod2_fixed
    )
...
@pytest.mark.parametrize(
    ("kwargs", "fixed_code", "verbose_messages"),
    [
        (
        ...
        (
            {
                "code": code_mod_commented_star,
                "repls": {".mod1": ["a"], ".mod2": ["c"], ".mod3": ["name"]},
                "file": "directory/mod_commented_star.py",
            },
            code_mod_commented_star_fixed,
            [
...
def test_replace_imports_verbose_messages(
    kwargs, fixed_code, verbose_messages, verbose_enabled, verbose_kwarg, capsys
):
...
def test_replace_imports_warnings(capsys):
    ...
    assert (
        replace_imports(
            code_mod_commented_unused_star,
            file="module/mod_commented_unused_star.py",
            repls={".mod1": [], ".mod2": []},
        )
        == code_mod_commented_unused_star_fixed
    )
    ...
    assert (
        replace_imports(
            code_mod_commented_unused_star, quiet=True, repls={".mod1": [], ".mod2": []}
        )
        == code_mod_commented_unused_star_fixed
    )
    out, err = capsys.readouterr()
...
"""
    ...
    assert replace_imports(code, repls) == code_fixed.format(
        imp="""\
from reallyreallylongmodulename import (longname1, longname2, longname3, longname4, longname5,
    ...
    assert replace_imports(code, repls, max_line_length=49) == code_fixed.format(
        imp="""\
from reallyreallylongmodulename import (longname1,
    ...
    assert replace_imports(code, repls, max_line_length=50) == code_fixed.format(
        imp="""\
from reallyreallylongmodulename import (longname1,
                                        longname2,
                                        longname3,
    ...
    assert replace_imports(code, repls, max_line_length=51) == code_fixed.format(
        imp="""\
from reallyreallylongmodulename import (longname1,
                                        longname2,
                                        longname3,
...
@pytest.mark.parametrize(
    "case_permutation",
    [lambda s: s, lambda s: s.upper(), lambda s: s.lower()],
...
def test_cli(tmpdir):
    from rmstar.__main__ import __file__
    ...
    p = subprocess.run(
        [sys.executable, "-m", "rmstar", "--_this-file", "none"],
        capture_output=True,
        encoding="utf-8",
    )
    assert p.stderr == ""
    ...
    p = subprocess.run(
        [sys.executable, "-m", "rmstar", directory],
        capture_output=True,
        encoding="utf-8",
    )
    warnings = set(
...
""",
    ...
    p = subprocess.run(
        [sys.executable, "-m", "rmstar", "--quiet", directory],
        capture_output=True,
        encoding="utf-8",
    )
    assert p.stderr == ""
    ...
    p = subprocess.run(
        [sys.executable, "-m", "rmstar", "--verbose", directory],
        capture_output=True,
        encoding="utf-8",
    )
    changes = set(
...
""".splitlines()
    ...
    p = subprocess.run(
        [sys.executable, "-m", "rmstar", "--no-dynamic-importing", directory],
        capture_output=True,
        encoding="utf-8",
    )
    static_error = set(
...
""".splitlines()
    ...
    p = subprocess.run(
        [sys.executable, "-m", "rmstar", "--quiet", "-i", directory],
        capture_output=True,
        encoding="utf-8",
    )
    assert p.stderr == ""
    ...
    assert cmp.diff_files == [
        ...
        "mod7.py",
        "mod9.py",
        "mod_commented_star.py",
        "mod_commented_unused_star.py",
    ]
    assert cmp.subdirs["submod"].diff_files == [
        "submod1.py",
        "submod2.py",
        "submod4.py",
    ]
    ...
    p = subprocess.run(
        [sys.executable, "-m", "rmstar", directory / "notarealfile.py"],
        capture_output=True,
        encoding="utf-8",
    )
    assert (
